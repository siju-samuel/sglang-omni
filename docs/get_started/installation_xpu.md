...
# platform-layer unit tests (CPU, no GPU) — needs pytest, which ships in the
# `[eval]` extra (install with `.[eval]`, or `pip install pytest` first)
pytest tests/unit_test/platforms/test_platforms.py -v

# device enumeration, and that the accelerator is the resolved platform
sgl-omni check-gpu
```
...
No extra environment variables are needed — `--device xpu` selects the backend. If a Triton JIT
...
```bash
sgl-omni serve --device xpu --model-path /path/to/Qwen3-ASR-1.7B --host 0.0.0.0 --port 8000
# transcribe:
...
```bash
sgl-omni serve --device xpu --model-path /path/to/Qwen3-TTS-12Hz-1.7B-Base --host 0.0.0.0 --port 8000
...
sgl-omni serve --device xpu --model-path /path/to/Qwen3-Omni-30B-A3B-Instruct \
