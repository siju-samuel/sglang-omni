...
# Every request below is loopback. A runner that exports HTTP_PROXY would send
# them to the proxy instead -- requests exempts nothing on its own, not even
# 127.0.0.1 -- and the refusal reads as a broken server. The launcher declines the
# same environment through start_server_from_cmd(strip_proxy=True).
