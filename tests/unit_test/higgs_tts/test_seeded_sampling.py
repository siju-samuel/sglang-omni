...
pytestmark = pytest.mark.skipif(
    ACCELERATOR is None, reason="multinomial_with_seed needs an accelerator"
)
...
def _logits(seed):
    g = torch.Generator(device=ACCELERATOR).manual_seed(seed)
    return torch.randn(B, N, V, generator=g, device=ACCELERATOR)


def _draw(logits, seeds_B, step):
    return _sample_independent_batched(
        logits,
        temperature=torch.ones(B, device=ACCELERATOR),
        top_p=None,
        seeds_B=torch.tensor(seeds_B, device=ACCELERATOR),
        step_B=torch.full((B,), step, dtype=torch.long, device=ACCELERATOR),
    )
...
def test_per_row_seed_isolation():
    ...
    alone = _sample_independent_batched(
        logits[:1],
        temperature=torch.ones(1, device=ACCELERATOR),
        top_p=None,
        seeds_B=torch.tensor([55], device=ACCELERATOR),
        step_B=torch.full((1,), 3, dtype=torch.long, device=ACCELERATOR),
    )
...
def test_seeded_sampler_preserves_probability_distribution():
    batch = 20_000
    probs = torch.tensor([0.9, 0.1], device=ACCELERATOR)
    logits = probs.log().view(1, 1, 2).expand(batch, 1, 2).contiguous()
    sampled = _sample_independent_batched(
        logits,
        temperature=torch.ones(batch, device=ACCELERATOR),
        top_p=None,
        seeds_B=torch.arange(1, batch + 1, device=ACCELERATOR),
        step_B=torch.zeros(batch, dtype=torch.long, device=ACCELERATOR),
    )
